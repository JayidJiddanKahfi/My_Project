...
class PaymentController extends Controller
{
    ...
    public function create_first_version(Request $request){
    ...
    public function create_second_version(Request $request){
        //membuat kumpulan rule (aturan) awal untuk validasi request
        $rules = [           
            "resident_id" => ["required","exists:residents,id"],
            "payment_date" => ["required","date"],
    ...
    public function read_for_payments(){
        //mengambil semua data di kolom pada semua baris dengan menambahkan data relasi dan mereturn dalam bentuk objek payments
        $payments = Payment::select(['resident_id','payment_date','recorded_by',Payment::raw('CONCAT_WS(" to ",MIN(payment_month),MAX(payment_month)) as month, SUM(contribution) as total')])->groupBy(['resident_id','payment_date','recorded_by'])->with('resident')->with('user')->get();

        //mereturn response json berupa data dari objek payments
        return response()->json(["paymentsData" => $payments]);
    }

    public function read_for_contributions(){
        ...
        for($i = 0; $i < count($payments); $i++){
            $payments[$i]->contribution_amount = $fee->contribution_amount;

            $payments[$i]->social_fund = $fee->social_fund;
        }
